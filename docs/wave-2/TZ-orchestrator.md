...
> **Статус 2026-07-11:** реализовано в `services/orchestrator/`, документация —
> `docs/orchestrator.md`, локально зелёные `make build`, `make vet`, `make test`.
> Операторские хвосты остаются вне объёма: живой cloud apply и vantage-пробы из РФ.

Оркестратор был недостающим звеном, замыкающим петлю антихрупкости: до Волны 2 это
была заглушка (`/healthz`). Вокруг уже были готовы швы: telemetry отдаёт
рекомендации, infra даёт скрипты жизненного цикла ноды, control-plane хранит
`Node`. Задача ТЗ — соединить их в автоматический контур «блок → замена ноды →
юзеры переехали».
