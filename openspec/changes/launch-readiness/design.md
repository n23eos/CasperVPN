...
9. Публичные FieldSignal являются непроверенными наблюдениями. Они не создают действия; автоматические решения требуют аутентифицированных HealthEvent. Limiter доверяет XFF только от явно заданного proxy CIDR.
10. Legacy ручные выдачи и изменения аккаунтов доступны только admin; billing использует ensure-subscription и versioned billing-state. Частичный status PATCH сохраняет Telegram identity и остальные поля аккаунта.
11. Publish сначала проверяет active fleet, публичный DNS/TLS и точный ответ subscription, затем включает Telegram. До этого запускаются только приватные API.
12. Клиентский singbox JSON и ноды проверяются на sing-box 1.14.2. DNS использует typed server и явный default_domain_resolver; старое ядро 1.11.11 не поддерживает этот JSON. Base64 и Clash сохраняют прежние форматы. Минимальная проверенная версия указывается в runbook; release gate запускает реальный sing-box check и соединения VLESS/Hy2.
