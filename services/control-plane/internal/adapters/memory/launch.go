...
func (r *Subscriptions) ApplyBillingState(_ context.Context, id string, state contracts.BillingState) (contracts.Subscription, error) {
	...
	incoming := domain.BillingFingerprint(state, s.Plan)
	if state.Revision == s.BillingRevision {
		if fingerprint := r.billingHashes[id]; fingerprint != "" {
			if fingerprint != incoming {
				return s, domain.ErrConflict
			}
		} else if (state.Plan != "" && s.Plan != state.Plan) || state.Status != s.Status || s.ExpiresAt == nil || !state.ExpiresAt.Equal(*s.ExpiresAt) || s.GraceUntil == nil || !state.GraceUntil.Equal(*s.GraceUntil) {
			return s, domain.ErrConflict
		}
	}
	if state.Revision > s.BillingRevision {
		if state.Plan != "" {
			domain.ApplyPlanLimits(&s, state.Plan)
		}
		r.billingHashes[id] = incoming
...
func (r *Subscriptions) CancelSubscription(_ context.Context, id string) (contracts.Subscription, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	s, ok := r.subs[id]
	if !ok {
	...
	s.Status = contracts.SubscriptionStatusCanceled
	s.UpdatedAt = time.Now()
	r.subs[id] = s
	return s, nil
}
