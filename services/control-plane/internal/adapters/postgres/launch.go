...
func (s *SubscriptionStore) ApplyBillingState(ctx context.Context, id string, state contracts.BillingState) (contracts.Subscription, error) {
	...
	err := withTx(ctx, s.pool, func(q querier) error {
		var locked, fingerprint string
		if err := q.QueryRow(ctx, `SELECT id,billing_state_hash FROM subscriptions WHERE id=$1 FOR UPDATE`, id).Scan(&locked, &fingerprint); err != nil {
		...
		incoming := domain.BillingFingerprint(state, current.Plan)
		if current.BillingRevision == state.Revision {
			if fingerprint != "" {
				if fingerprint != incoming {
					return domain.ErrConflict
				}
			} else if (state.Plan != "" && current.Plan != state.Plan) || current.Status != state.Status || current.ExpiresAt == nil || !current.ExpiresAt.Equal(state.ExpiresAt) || current.GraceUntil == nil || !current.GraceUntil.Equal(state.GraceUntil) {
				return domain.ErrConflict
			}
		} else if current.BillingRevision < state.Revision {
			if state.Plan != "" {
				domain.ApplyPlanLimits(&current, state.Plan)
			}
			if _, err := q.Exec(ctx, `UPDATE subscriptions SET billing_revision=$2,status=$3,expires_at=$4,grace_until=$5,plan=$6,traffic_limit_bytes=$7,speed_limit_mbps=$8,device_limit=$9,billing_state_hash=$10,updated_at=now() WHERE id=$1`, id, state.Revision, string(state.Status), state.ExpiresAt, state.GraceUntil, string(current.Plan), int64(current.TrafficLimitBytes), current.SpeedLimitMbps, current.DeviceLimit, incoming); err != nil {
				return err
			}
		}
...
func (s *SubscriptionStore) CancelSubscription(ctx context.Context, id string) (contracts.Subscription, error) {
	var result contracts.Subscription
	err := withTx(ctx, s.pool, func(q querier) error {
		tag, err := q.Exec(ctx, `UPDATE subscriptions SET status='canceled',updated_at=now() WHERE id=$1`, id)
		if err != nil {
			return err
		}
		...
		result, err = (&SubscriptionStore{q: q}).Get(ctx, id)
		return err
	})
	return result, err
}
