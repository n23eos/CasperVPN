...
func (s *ScriptRunner) SyncAccess(ctx context.Context, node contracts.Node, snapshot contracts.NodeAccessUsers) error {
	...
	realityNames, realityDest, hy2SNI, err := nodeTransportContext(node)
	if err != nil {
		return err
	}
	return s.run(ctx, "access_sync.sh", map[string]string{
		"RUN_ID": runID, "RUN_DIR": s.runDir(), "NODE": node.ID,
		"ACCESS_SNAPSHOT_FILE": path, "HY2_SNI": hy2SNI,
		"REALITY_SERVER_NAMES": realityNames, "REALITY_DEST": realityDest,
	})
}

func nodeTransportContext(node contracts.Node) (string, string, string, error) {
	var realityNames, realityDest, hy2SNI string
	for _, transport := range node.Transports {
		...
		case contracts.TransportVlessReality:
			if transport.VlessReality != nil {
				realityNames = strings.Join(transport.VlessReality.ServerNames, ",")
				realityDest = strings.TrimSpace(transport.VlessReality.Dest)
			}
		case contracts.TransportHysteria2:
			if transport.Hysteria2 != nil {
				hy2SNI = strings.TrimSpace(transport.Hysteria2.SNI)
			}
		}
	}
	if realityNames == "" || realityDest == "" || hy2SNI == "" {
		return "", "", "", fmt.Errorf("provision: active node %s lacks enabled VLESS/REALITY or Hysteria2 transport context", node.ID)
	}
	return realityNames, realityDest, hy2SNI, nil
}
