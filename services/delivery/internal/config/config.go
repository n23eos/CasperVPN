...
// Every channel endpoint, key and zone is config-supplied. Production fails
// closed; an explicit dev/test mode may boot with the bot disabled. No domain,
// IP, mirror or key is hardcoded in the binary (CLAUDE.md [ANTI-BLOCK] rule 5): unset
...
func Load() (Config, error) {
	...
	cfg := Config{
		...
		SignKeyID:         e.Str("DELIVERY_SIGN_KEY_ID", "delivery-v1"),
		...
		TelegramBase:  strings.TrimRight(e.Str("DELIVERY_TELEGRAM_BASE", ""), "/"),
		TelegramToken: e.Str("DELIVERY_TELEGRAM_TOKEN", ""),
		MaxBase:       strings.TrimRight(e.Str("DELIVERY_MAX_BASE", ""), "/"),
...
func (c Config) Validate() error {
	...
	if c.SignSeedB64 != "" && !validBase64Key(c.SignSeedB64) {
		return fmt.Errorf("config: DELIVERY_SIGN_SEED must be base64 for 32 bytes")
	}
	if c.SealKeyB64 != "" && !validBase64Key(c.SealKeyB64) {
		return fmt.Errorf("config: DELIVERY_SEAL_KEY must be base64 for 32 bytes")
	}
	if !c.BotEnabled {
		return nil
	}
...
func validBase64Key(raw string) bool {
