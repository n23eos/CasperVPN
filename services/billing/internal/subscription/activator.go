...
func (a *Activator) CreditInvoice(ctx context.Context, inv model.Invoice) (contracts.Subscription, error) {
	...
	billingCP, ok := a.cp.(controlplane.BillingClient)
	if !ok {
		return contracts.Subscription{}, fmt.Errorf("subscription: billing-state API unavailable")
	}
	err := a.store.WithUserLock(ctx, inv.AnonUserID, func(ctx context.Context) error {
		sub, err := billingCP.EnsureSubscription(ctx, inv.AnonUserID, contracts.SubscriptionPlan(inv.Plan))
		if err != nil {
...
func (a *Activator) Deliver(ctx context.Context, delivery model.BillingDelivery) (contracts.Subscription, error) {
	...
	state.Plan = contracts.SubscriptionPlan(delivery.Plan)
	sub, err := billingCP.SetBillingState(ctx, delivery.SubID, state)
	if err != nil {
